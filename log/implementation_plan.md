...
| **Client-Side 통계적 필터링 + 패턴 압축** (QA-3) | 수행 효율성 ↑ (Token 절감) | 기능 정확성 ↓ (중요 단서 누락 위험, False Negative) | `get_context` 도구로 에이전트가 추가 컨텍스트를 요청할 수 있도록 보완하여 완화 |
...
| QA | 테스트 항목 | 검증 방법 | 합격 기준 |
|----|-----------|----------|----------|
| QA-1 | 라인 참조 정확도 | LLM 결과의 모든 라인 번호를 원본 파일 내 유효성 검증 | 유효하지 않은 참조 = 0건 |
| QA-1 | RCA 정확도 벤치마크 | 통신/모바일/가전 등 10개 대표 에러 로그로 정확도 측정 | 정확도 ≥ 85% |
| QA-2a | 비정형 포맷 감지 | logcat, syslog 및 다양한 사내 독자 포맷 샘플로 동적 감지 | 감지 성공률 ≥ 90% |
| QA-2a | 도메인 메모리 반영 | `teach_domain` 으로 주입된 지식이 다음 추론에 반영되는지 확인 | 반영률 100% (Global 저장소 쓰기 검증 포함) |
| QA-2a | 제품군 교차 검증 | 모바일 환경 지식이 TV나 통신 장비 로그 분석 시 잘못 적용되는지 검사 | 오적용 0건 |
