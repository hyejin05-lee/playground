---
name: Architecture_Verification
model: inherit
...
1. **하향식 일관성 및 추적성 검증 (Top-Down Consistency & Traceability)**:
   - `Architectural Driver` -> `QAS` -> `Candidate Design (CDA)` -> `Design Decision` -> `Architecture Design` -> `Detailed Design` -> `Traceability Matrix` 전체 흐름의 연속성을 검증한다.
